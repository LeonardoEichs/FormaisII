...
public class AddCFG extends JFrame {
	 ...
	 */
	public AddCFG(MainFrame mainFrame) {
		try {
	...
	private void initialize() {
		...
		JLabel lblAddRLAdd = new JLabel("Context Free Grammar:");
		...
		btnAddRLAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(name.isEmpty() || name.contains("[") || name.contains("]")) {
					JOptionPane.showMessageDialog(AddCFG.this, "Invalid name");
					return;
				}
				else if (AddCFG.this.mainFrame.getLanguage(name) != null) {
				...
				ContextFreeGrammar cfg = ContextFreeGrammar.isValidCFG(input);
				if(cfg == null) {
					JOptionPane.showMessageDialog(AddCFG.this, "Invalid input!");
					return;
				}
				cfg.setId(name);
				AddCFG.this.mainFrame.addToPanel(cfg);
				AddCFG.this.exit();
			}
		});
